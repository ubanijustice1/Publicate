...
- **Trend data:** Google Trends Scraper on Apify (via Netlify Functions)
...
The `VITE_*` vars are safe to expose in the browser. `OPENAI_API_KEY`, `APIFY_API_TOKEN`, `SUPABASE_SERVICE_ROLE_KEY` and `PAYSTACK_SECRET_KEY` are server-only secrets used exclusively inside `netlify/functions/*` — never import them into `src/`.
